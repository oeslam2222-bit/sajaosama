...
import { Location, Driver, Trip, Rider, Region } from '../types';
...
import { calculateHaversineDistance, estimateDrivingDistance, calculateDynamicFare, getVehiclePricing, calculateVehicleFare } from '../utils/haversine';
...
export const RiderView: React.FC<RiderViewProps> = ({
  ...
  const recentDropoffs = tripsHistory
  ...
  const loadMyTrips = async (reset = false) => {
    ...
    try {
      const allTrips = tripsHistory.filter(t => t.riderId === rider.id);
      const start = reset ? 0 : myTripsPage * 10;
  ...
  useEffect(() => {
    loadMyTrips(true);
  }, [rider.id, myTripDateFrom, myTripDateTo]);
  ...
  // Calculate estimated distance and fare using simplified model
  ...
  const incomingCommissionPercent = stats?.incomingCommissionPercent ?? 10;
  const commissionMode = stats?.commissionMode || 'fixed';

  const computeTripFare = (dist: number, vehicleType: string) => {
    const pricing = getVehiclePricing(stats, vehicleType);
    const baseFare = calculateVehicleFare(dist, pricing);
    const commission =
      commissionMode === 'percent'
        ? Math.round((baseFare * incomingCommissionPercent) / 100)
        : Math.round(incomingCommission);
  ...
  const calculateFareForLocation = (dropoff: { lat: number; lng: number }): number => {
    ...
    return computeTripFare(finalDist, 'CAR');
  };
  ...
  if (pickupLoc && dropoffLoc) {
    ...
    estimatedFare = computeTripFare(distance, requestedVehicleType);
  ...
  const handleSetWork = () => {
   ...
   return (
    <div className="flex flex-col h-full bg-slate-50 text-slate-900 select-none font-sans" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      {/* Modern Top Header / Rider Profile Card */}
      <div className="bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 py-3 flex items-center justify-between sticky top-0 z-40 shadow-xs">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-purple-600 text-white flex items-center justify-center font-black text-sm shadow-md shadow-blue-500/20">
              {rider.name ? rider.name[0] : 'U'}
            </div>
            <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-emerald-500 border-2 border-white rounded-full" />
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <h4 className="text-xs font-black text-slate-900 leading-none">{rider.name || (lang === 'ar' ? 'الراكب' : 'Rider')}</h4>
            ...
            <p className="text-[10px] text-slate-400 font-medium mt-1">{rider.phone}</p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          {/* Wallet Balance Badge */}
          <div className="bg-slate-900 text-white px-3 py-1.5 rounded-xl shadow-xs flex items-center gap-1.5">
            <span className="text-[10px] text-slate-400 font-medium">{lang === 'ar' ? 'الرصيد:' : 'Balance:'}</span>
            <span className="text-xs font-black text-amber-300 font-mono">{rider.balance} {lang === 'ar' ? 'ج.م' : 'EGP'}</span>
          </div>
          ...
          <button
            ...
            title={lang === 'ar' ? (lowDataMode ? 'وضع توفير البيانات مفعل' : 'تفعيل توفير البيانات') : (lowDataMode ? 'Low Data Active' : 'Save Data')}
          >
            <span className="text-xs">📡</span>
            <span className="hidden sm:inline">{lang === 'ar' ? (lowDataMode ? 'توفير البيانات' : 'توفير') : (lowDataMode ? 'Low Data' : 'Save')}</span>
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4">
        ...
        {activeTrip && activeTrip.riderId === rider.id && activeTrip.status === 'COMPLETED' && !activeTrip.riderRatingToDriver && (
        ...
        {(!activeTrip || activeTrip.riderId !== rider.id) && (
          <div className="space-y-4">
            ...
            {/* Recent Trips / Last Destination */}
            {recentDropoffs.length > 0 && (
              <div className="bg-white border border-slate-200 rounded-2xl p-3 space-y-2 text-right">
                <p className="text-[10px] font-extrabold text-slate-500 flex items-center gap-1 justify-end">
                  <span>{lang === 'ar' ? '🕒 وجهاتك الأخيرة' : '🕒 Recent destinations'}</span>
                </p>
                <div className="flex flex-wrap gap-1.5 justify-end">
                  {recentDropoffs.map((dropoff) => (
                    <button
                      ...
                      onClick={() => {
                        if (pickupLoc) {
                          setSelectedDropoff(dropoff.id);
                        }
                      }}
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 text-[10px] bg-slate-50 text-slate-700 hover:bg-rose-50 hover:text-rose-700 border border-slate-200 hover:border-rose-200 rounded-full transition-all cursor-pointer pointer-events-auto"
                    >
                      <span>🏁</span>
                      <span className="font-bold">{dropoff.name}</span>
                      {pickupLoc && (
                        <span className="text-[9px] text-slate-400">
                          {calculateFareForLocation({ lat: dropoff.lat, lng: dropoff.lng })} {lang === 'ar' ? 'ج.م' : 'EGP'}
                        </span>
                      )}
                    </button>
                  ))}
              ...
              <div className="space-y-1.5" dir="rtl">
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {[
                    ...
                    const vFare = computeTripFare(distance, v.id);
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-2xl font-black text-amber-500 block leading-tight">
                      {appliedPromo ? (
                        <>
                          <span className="text-xs line-through text-slate-400 mr-1">{estimatedFare}</span>
                          <span>{Math.max(1, estimatedFare - appliedPromo.discount)}</span>
                        </>
                      ) : (
                        estimatedFare
                      )} {lang === 'ar' ? 'ج.م' : 'EGP'}
